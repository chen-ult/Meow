...
public class UI_TreeConnection : MonoBehaviour
{
    ...
    public void DirectConnection(NodeDirectionType direction, float lenght, float offset)
    {
        bool shouldBeActive = direction != NodeDirectionType.None;
        float finalLenght = shouldBeActive ? lenght : 0;
        float angle = GetDirectionAngle(direction);


        rotationPoint.localRotation = Quaternion.Euler(0, 0, angle + offset);
        connectionLength.sizeDelta = new Vector2(finalLenght, connectionLength.sizeDelta.y);
    }
